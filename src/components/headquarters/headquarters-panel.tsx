...
import { formatDate, graphData, inProject, kindNames, priorityNames, priorityTasks, projectNames, statusNames, taskProjectName, type ProjectFilter } from './hq-data'
...
const views: { id: HQView; name: string; icon: string }[] = [{ id: 'overview', name: 'Oversikt', icon: '◫' }, { id: 'projects', name: 'Prosjekter', icon: '▦' }, { id: 'knowledge', name: 'Kunnskap', icon: '⌘' }, { id: 'decisions', name: 'Beslutninger', icon: '⑂' }, { id: 'analysis', name: 'Analyse', icon: '▥' }]
const introductions: Record<HQView, { title: string; detail: string }> = {
  projects: { title: 'Et tydelig rom for hvert prosjekt.', detail: 'Mål, team, arbeid og dokumenterte resultater fra det samme prosjektregisteret.' },
...
export function HeadquartersPanel() {
  ...
  const [projectManager, setProjectManager] = useState<{ id?: number } | null>(null)
  ...
  const focusedId = project.startsWith('project:') ? Number(project.slice(8)) : undefined
  const focusedResource = useHQResource<HQSnapshot>(focusedId ? `/api/headquarters?projectId=${focusedId}` : null, refresh, 60_000)
  const data = snapshot.data
  const focusedProject = (focusedResource.data?.projects || data?.projects || []).find(item => item.id === focusedId)
  const knowledgeProject = focusedProject?.key || (focusedId ? 'shared' : undefined)
  const projectLabel = focusedProject?.name || 'Alle prosjekter'
  const delayedQuery = useDebouncedValue(query.trim())
  const searchUrl = delayedQuery ? `/api/headquarters/knowledge?q=${encodeURIComponent(delayedQuery)}${knowledgeProject ? `&project=${knowledgeProject}` : ''}` : null
  const search = useHQResource<HQSearchResponse>(searchUrl, refresh)
  const metrics = useHQResource<HQMetricsResponse>((view === 'analysis' || (view === 'projects' && focusedProject)) && (!focusedId || focusedProject?.key) ? `/api/headquarters/metrics${knowledgeProject ? `?project=${knowledgeProject}` : ''}` : null, refresh, 60_000)
  const notes = useMemo(() => (data?.notes || []).filter(note => inProject(note, project, data?.projects)), [data, project])
  const tasks = useMemo(() => (focusedId ? focusedResource.data?.tasks || [] : data?.tasks || []).filter(task => inProject(task, project, data?.projects)), [data, project, focusedId, focusedResource.data])
  const filteredNotes = useMemo(() => {
    if (!query.trim()) return notes
    if (delayedQuery === query.trim() && search.data && !search.error) return search.data.notes.filter(note => inProject(note, project, data?.projects))
  ...
  }, [notes, query, delayedQuery, search.data, search.error, project, data])
  ...
  useEffect(() => { if (data && focusedId && !data.projects.some(item => item.id === focusedId)) { setProject('all'); setView('projects') } }, [data, focusedId])
  ...
  const initialProject: HQProjectKey = focusedProject?.key || composer?.note?.projectKey || 'shared'
  const initialProjectId = focusedProject?.id ?? data?.projects.find(item => item.key === initialProject)?.id ?? undefined
  const openProjectTasks = () => {
    if (!focusedProject) return
    useMissionControl.getState().setActiveProject({ id: focusedProject.id, name: focusedProject.name, slug: focusedProject.slug || '', ticket_prefix: focusedProject.ticketPrefix || '', status: 'active' })
    router.push(panelHref('tasks'), { scroll: false })
  }
  const manageProject = (id?: number) => setProjectManager({ id })
  return <div className={styles.hq}>
    ...
    <div className={styles.navigation}><nav aria-label="Hovedkvarterets visninger">{views.map(tab => <button key={tab.id} onClick={() => setView(tab.id)} aria-current={view === tab.id ? 'page' : undefined}><span aria-hidden="true">{tab.icon}</span>{tab.name}</button>)}</nav><label className={styles.projectSelect}><span>Prosjekt</span><select value={project} onChange={event => { setProject(event.target.value as ProjectFilter); if (event.target.value !== 'all' && view === 'overview') setView('projects') }}><option value="all">Alle prosjekter</option>{data?.projects.map(item => <option key={item.id} value={`project:${item.id}`}>{item.name}</option>)}</select></label></div>
    <div className={styles.intro}><div><span className={styles.overline}>{projectLabel} · {view === 'analysis' ? 'Resultater og datagrunnlag' : 'Arbeidsrom'}</span><h1>{view === 'projects' && focusedProject ? `${focusedProject.name}. Mål, team og fremdrift.` : introductions[view].title}</h1><p>{introductions[view].detail}</p></div><button className={styles.button} onClick={() => setComposer({})}>＋ Ny MC-oppgave</button></div>
    ...
    {!data && snapshot.loading ? <div className={`${styles.card} ${styles.empty}`} role="status"><span className={styles.loadingGlyph} aria-hidden="true">◌</span><strong>Samler kilder, oppgaver og drift</strong><p>Arbeidsrommet fylles fra de faktiske datakildene.</p></div> : !data ? <Sources sources={[]} /> : <>
      ...
      {focusedId && focusedResource.loading && <p className={styles.muted} role="status">Oppdaterer prosjektets oppgaver …</p>}
      {focusedResource.error && <div className={styles.error} role="alert">Prosjektets oppgaver kunne ikke oppdateres: {focusedResource.error}{focusedResource.data && <p>Sist hentede prosjektdata vises. De er ikke bekreftet oppdaterte.</p>}</div>}
      <div className={styles.mainGrid}><div className={styles.mainColumn}>
        {view === 'projects' && focusedProject && <ProjectWorkspace project={focusedProject} notes={filteredNotes} tasks={filteredTasks} agents={data.agents} metrics={metrics} onManage={() => manageProject(focusedId)} onSelect={select} onOpenTask={openTask} onTasks={openProjectTasks} onKnowledge={() => setView('knowledge')} onCreate={() => setComposer({})} />}
    ...
    {projectManager && <ProjectManagerModal initialProjectId={projectManager.id} onClose={() => setProjectManager(null)} onChanged={async () => { refreshAll() }} />}
    {composer && <TaskComposer projects={data?.projects} initialProjectId={initialProjectId} notes={data?.notes || notes} selectedNote={composer.note} initialProject={initialProject} onClose={() => setComposer(null)} onCreated={refreshAll} onOpenTask={openTask} />}
  </div>
}
