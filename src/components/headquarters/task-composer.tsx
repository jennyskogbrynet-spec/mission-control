...
import type { HQNote, HQProject, HQProjectKey, HQTask, HQTaskCreateInput } from '@/lib/hq-types'
...
export function TaskComposer({ notes, selectedNote, initialProject, initialProjectId, projects, onClose, onCreated, onOpenTask }: { notes: HQNote[]; projects?: HQProject[]; initialProjectId?: number; selectedNote?: HQNote; initialProject: HQProjectKey; onClose: () => void; onCreated: (task: HQTask) => void; onOpenTask: (task: HQTask) => void }) {
  ...
  const [legacyProject, setLegacyProject] = useState(initialProject)
  const project = projects?.find(item => item.id === projectId)?.key || (projects ? 'shared' : legacyProject)
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (projects && !projectId) { setError('Velg et aktivt prosjekt før lagring.'); return }
    const input: HQTaskCreateInput = { ...(projectId ? { projectId } : {}), title: title.trim(), description: description.trim(), projectKey: project, sourceIds, acceptanceCriteria: criteria.split('\n').map(line => line.trim()).filter(Boolean), expectedOutcome: outcome.trim(), priority, idempotencyKey: key.current }
